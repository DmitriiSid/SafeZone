...
print("db_pomoci_flagged_new_V2", db_pomoci_flagged_new_V2.shape)
...
#     lambda row: row['New Contact'] if pd.isna(row['E-mail']) and pd.notna(row['New Contact']) else row['E-mail'], axis=1)
# db_pomoci['Telefon'] = db_pomoci.apply(
#     lambda row: row['New Contact'] if pd.isna(row['Telefon']) and pd.notna(row['New Contact']) else row['Telefon'], axis=1)
db_pomoci['E-mail'] = db_pomoci.apply(
